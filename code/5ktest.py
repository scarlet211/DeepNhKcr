...
def Calculate_Kfold_mean_std_metrics_values(total_SN,total_SP,total_ACC,total_F1_score,total_MCC,total_AUC,total_AUP):
    ...
    print("5Kfold_Valid_Std_metrics : SN is {:.4f},SP is {:.4f},ACC is {:.4f},F1-score is {:.4f},MCC is {:.4f},AUC is {:.4f},AUP is {:.3f}".
          format(std_SN, std_SP, std_ACC, std_F1_score, std_MCC, std_AUC,std_AUP))

def cross_validation_train(model,epochs,train_loader,optimizer,train_criterion, fold,device,valid_loader,test_loader):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) == epochs:
            # save model
            torch.save(model.state_dict(), '../results/modweights/'+str(fold) + '_DeepNhKcr_kfold_model.pth'.format(fold))
...
def cross_validation_main(epochs,train_criterion,criterion):
    # kf = StratifiedKFold(n_splits=5, shuffle=True,random_state=3000)
    kf = StratifiedKFold(n_splits=5, shuffle=True, random_state=1800)
    # kf = StratifiedKFold(n_splits=5, shuffle=True, random_state=1200)
    fold = 1


    for train_index, valid_index in kf.split(train_set,train_set.labels):
        ...
        # training:
        cross_validation_train(model,epochs,train_loader,optimizer,train_criterion,fold,device,valid_loader,test_loader)
        # # test:
    ...
    Calculate_Kfold_mean_std_metrics_values(valid_total_SN, valid_total_SP, valid_total_ACC, valid_total_F1_score, valid_total_MCC, valid_total_AUC,valid_total_AUP)
