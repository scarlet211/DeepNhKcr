...
def Calculate_Kfold_mean_std_metrics_values(total_SN,total_SP,total_ACC,total_F1_score,total_MCC,total_AUC,total_AUP):
    ...
    print("Mean_metrics : SN is {:.3f},SP is {:.3f},ACC is {:.3f},F1-score is {:.3f},MCC is {:.3f},AUC is {:.3f},AUP is {:.3f}".
        format(mean_SN, mean_SP, mean_ACC, mean_F1_score, mean_MCC, mean_AUC,mean_AUP))
    print("Std_metrics : SN is {:.4f},SP is {:.4f},ACC is {:.4f},F1-score is {:.4f},MCC is {:.4f},AUC is {:.4f},AUP is {:.3f}".
          format(std_SN, std_SP, std_ACC, std_F1_score, std_MCC, std_AUC,std_AUP))

def cross_validation_train(model,epochs,train_loader,optimizer,train_criterion,device,run_idx):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) == epochs:
            # save model
            torch.save(model.state_dict(), '../anewresults/'+str(run_idx) + '_DeepNhKcr_test1_model.pth'.format(run_idx))
...
def cross_validate_test1(model, valid_loader, criterion, device):
    ...
    with torch.no_grad():
        ...
        for batch_id, data in enumerate(valid_loader):
            ...
            y_sco = torch.softmax(y_predict, dim=1)
            y_score.append(y_sco[:, 1].detach().cpu().numpy())
...
def cross_validation_main(epochs,train_criterion,criterion):
    ...
    for run_idx  in range(5):
        ...
        # training:
        cross_validation_train(model, epochs, train_loader, optimizer, train_criterion,  device,run_idx)
        # # test:
    ...
    print("最后结果>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    np.savez_compressed("../anewresults/p/allscores.npz", y_score=PSCORE)
